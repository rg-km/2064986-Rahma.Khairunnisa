...
func (u *TransactionRepository) Pay(cartItems []CartItem, amount int) (int, error) {
	...
	for _, cartItem := range cartItems {
		...
		sqlStmt := `UPDATE products SET quantity = quantity - ? WHERE id = ?`
		// HINT: use `tx.Exec("... query ...")` instead of `u.db.Exec("... query ...")`
		// TODO: answer here
		_, err = tx.Exec(sqlStmt, cartItem.Quantity, cartItem.ProductID)
		if err != nil {
			tx.Rollback()
			return 0, err
		}
		// TODO: Implement SQL Query using the transaction connection to add cart item to sales table
		sqlStatement := `INSERT INTO sales (date, product_id, quantity) VALUES
		(?, ?, ?)`
		// HINT: use `tx.Exec("... query ...")` instead of `u.db.Exec("... query ...")`
		// TODO: answer here
		_, err = tx.Exec(sqlStatement, time.Now(), cartItem.ProductID, cartItem.Quantity)
		if err != nil {
			tx.Rollback()
			return 0, err
		}
